...
haskell-nix: src: inputMap: haskell-nix.cabalProject' {
  ...
  # No source-repository-package patches needed for cardano-node 11.0.1;
  # all previous patches (cardano-ledger, ouroboros-network, ouroboros-consensus)
  # are now upstream.
  ...
  modules = [
    ...
    ({ pkgs, ... }: {
      ...
      packages = {
        ...
        # Use serial block IO to avoid requiring liburing
        blockio.flags.serialblockio = true;
      };
    })
  ];
}
